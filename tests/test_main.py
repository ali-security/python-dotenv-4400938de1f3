...
def test_set_key_preserves_file_mode(tmp_path):
...
def test_set_key_symlink_not_followed_on_cross_device_rename(tmp_path):
    # The rewrite must not degrade into a copy that writes through the symlink
    # when a rename can't be performed as-is (the cross-device fallback).
    target = tmp_path / "target.env"
    target.write_text("a=x\n")
    symlink = tmp_path / ".env"
    symlink.symlink_to(target)
    ...
    with mock.patch("os.rename", side_effect=cross_device_rename):
        dotenv.set_key(symlink, "a", "y")
...
def test_unset_key_symlink_not_followed_on_cross_device_rename(tmp_path):
    # The rewrite must not degrade into a copy that writes through the symlink
    # when a rename can't be performed as-is (the cross-device fallback).
    target = tmp_path / "target.env"
    target.write_text("a=x\nsecret=keep-me\n")
    ...
    assert target.read_text() == "a=x\nsecret=keep-me\n"
    assert not symlink.is_symlink()
    assert symlink.read_text() == "secret=keep-me\n"
